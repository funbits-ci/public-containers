...
RUN apt update && apt install openssl wget xz-utils dh-autoreconf nasm autoconf automake libtool dpkg --yes \
...
RUN wget https://nixos.org/releases/nix/nix-${NIX_VERSION}/nix-${NIX_VERSION}-$(uname -m)-linux.tar.xz \
  ...
  && for i in $(seq 1 30); do useradd --no-create-home -u $((30000 + i)) -G nixbld nixbld$i ; done \
